	...
	return (
		<div className="grid grid-cols-[auto_1fr] gap-2 xxs:gap-4">
			...
			<div className="flex flex-col gap-1 justify-between overflow-hidden">
				<h2 className="xxs:text-lg font-bold text-wrap leading-tight xxs:leading-tight">
					{item.name}
				</h2>
				<p className="text-xs xxs:text-sm xs:text-base">{priceFormatter(item.price)}</p>
				<p className="text-xs xxs:text-sm xs:text-base text-slate-800/60">
					Qty {item.quantity}
				</p>
			</div>
		</div>
