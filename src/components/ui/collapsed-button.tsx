...
export default function CollapsedButton({ src, alt, expand, imageSize, rotate = true, children }: { src: StaticImageData | string, alt: string, expand: boolean, imageSize: string, rotate?: boolean, children: React.ReactNode }) {
    ...
    return (
        ...
        >
            <div className={`relative ${imageSizeClasses}`}>
                <Image src={src} alt={alt} fill sizes="100% 100%" draggable={false} className={`p-1 lg:p-2 transition-all duration-300 ${rotate && (isHovered || expand ? "md:rotate-0" : "md:-rotate-[45deg]")}`} />
            </div>
