...
export default function Reservation() {
	...
	defaultDate.setDate(
		defaultDate.getDate() +
			(defaultDate.getDay() === 0 ? 2 : defaultDate.getDay() === 1 ? 1 : 0)
	);
	// set time to 00:00:00
	defaultDate.setHours(0, 0, 0, 0);

	const [selectedDate, setSelectedDate] = useState<Value>(defaultDate);
	const { guests, selectedTime, readyToSubmit } = useAppSelector((state) => state.reservation);
	...
	return (
		...
		>
			<div className="max-w-[40rem]">
				...
				<p className="p-2 xs:p-4 text-justify xs:text-center text-pretty">
					Kindly be aware that all tables reserved before 7 pm (Tuesday - Thursday) / 8 pm
					(Friday & Saturday) will be allotted for a duration of 2 hours only. If you need
					additional time, please get in touch with us directly, and we will endeavour to
