...
export default function AddToCartButton({ item }: { item: MenuItem }) {
    ...
    const clickHandler = () => {
        ...
        dispatch(
            cartActions.setTimer1(
                setTimeout(() => dispatch(cartActions.setChanged(false)), 1000 * 1.25)
            )
        );
        dispatch(
            cartActions.setTimer2(
                setTimeout(() => dispatch(cartActions.resetAddedItems()), 1000 * 1.25 + 500)
            )
        );
    };
