...
const itemIds = ["bruschetta", "frittata", "spaghetti-bolognese", "risotto-ai-funghi", "cannoli"];
	...
	const heroImages = images.map((image) => {
		return {
			...
			reference: (
				<>
					Photo by{" "}
					<Link href={image.reference.owner_url} className="underline">
						{image.reference.owner}
					</Link>{" "}
					on{" "}
					<Link href={image.reference.url} className="underline">
