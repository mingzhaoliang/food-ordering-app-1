...
function FormInput({ label, id, type, pattern, defaultValue, readOnly, required }: { label: string, id: string, type: string, pattern?: string, defaultValue: string, readOnly?: boolean, required?: boolean }) {
    return (
        <div className="space-y-2">
            <label htmlFor={id} className={`block text-slate-800 ${required ? "after:content-['*'] after:ml-0.5 after:text-red-500" : ""}`}>{label}</label>
            <input type={type} id={id} name={id} required={required} className={`block w-full p-2 border border-gray-300 rounded-md outline-none ${readOnly ? "text-slate-500" : "text-slate-800"}`} pattern={pattern} defaultValue={defaultValue} readOnly={readOnly} />
        </div>
    )
}
...
export default function ProfilePage() {
    ...
    return (
        <>
            <form className="max-md:mx-auto max-xs:w-full max-sm:w-11/12 max-md:w-10/12 md:max-w-[50rem] p-6 pb-8 bg-white rounded-lg flex flex-col gap-4 font-lato max-md:text-md max-lg:text-base transition-all" action={formAction} >
                <h1 className="md:hidden text-xl font-bold">Profile</h1>
                <FormInput label="Name" id="username" type="text" defaultValue={user.username || ""} required />
                ...
                <div className="grid grid-cols-[repeat(auto-fit,_minmax(8rem,_1fr))] gap-4">
                    <div className="space-y-2">
                        ...
                        <select key={user.state} id="state" name="state" className="block w-full p-2 border border-gray-300 rounded-md outline-none" defaultValue={user.state || "Select a state"}>
                            <option value="Select a state" disabled hidden>Select a state</option>
                ...
                {state.status && state.status === "error" && <p className="text-rose-500">{state.message}</p>}
